  ...
  checkOnsetOffsetSyntax(tagGroup) {
    ...
    try {
      ...
      if (e instanceof IssueError) {
        ...
        definitionName = 'Multiple Def tags found'
      }
    }
    ...
    if (tagGroup.hasDefExpandChildren && tagGroup.isDefGroup) {
      this.pushIssue('onsetOffsetWithMultipleDefinitions', {
        tagGroup: tagGroup.originalTag,
      })
    } else if (!(tagGroup.hasDefExpandChildren || tagGroup.isDefGroup)) {
      this.pushIssue('onsetOffsetWithoutDefinition', {
        tagGroup: tagGroup.originalTag,
      })
    }
    if (tagGroup.isOnsetGroup) {
      const allowedTags = [getParsedParentTags(this.hedSchemas, 'Onset').get(this.hedSchemas.standardSchema)]
      if (defExpandChildren.length > 1) {
        this.pushIssue('onsetOffsetWithMultipleDefinitions', {
          tagGroup: tagGroup.originalTag,
        })
        allowedTags.push(...defExpandChildren)
      } else if (defExpandChildren.length === 1) {
        allowedTags.push(defExpandChildren[0])
      }
      if (defTags.length > 1) {
        /*
         This is generated by the try-catch above.
         this.pushIssue('onsetOffsetWithMultipleDefinitions', {
          tagGroup: tagGroup.originalTag,
        })
         */
        allowedTags.push(...defTags)
      } else if (defTags.length === 1) {
        allowedTags.push(defTags[0])
      }
      const remainingTags = differenceWith(tagGroup.tags, allowedTags, (ours, theirs) => {
        return ours.equivalent(theirs)
      })
      if (remainingTags.length > 1 || remainingTags.filter((tag) => tag instanceof ParsedHedTag).length > 0) {
        this.pushIssue('multipleTagsInOnset', {
          definition: definitionName,
        })
      }
    }
